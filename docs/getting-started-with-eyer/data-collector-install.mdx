...
Next, navigate to your InfluxDB Telegraf folder, typically located at `C:\Program Files\InfluxData\telegraf\telegraf-1.30.1\telegraf`. Replace the existing `telegraf.conf` file with the downloaded Eyer `telegraf.conf` file. Then, make the following edits to your Eyer `telegraf.conf` file:
