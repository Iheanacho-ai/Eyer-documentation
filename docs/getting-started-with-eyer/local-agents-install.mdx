...
4. Once you’ve made your changes to the `telegraf.conf` file. Register and start your Telegraf agent as a service with the following commands for your operating system.


    * For Windows users, change your current directory to the Telegraf folder. Then, move the `telegraf.exe` and `telegraf.conf` files from `C:\Program Files\InfluxData\telegraf\telegraf-1.30.1` to its parent directory `C:\Program Files\InfluxData\telegraf`:

        ```
        cd "C:\Program Files\InfluxData\telegraf";
        mv .\telegraf-1.30.1\telegraf.* .
        ```
        Next, install and start up your telegraf service with this command:
        ```
