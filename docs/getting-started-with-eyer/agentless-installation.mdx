...
4. Once you’ve made your changes to the `telegraf.conf` file. Register and start your Telegraf agent as a service by following the commands in the [Telegraf official documentation](https://docs.influxdata.com/telegraf/v1/install/).


    * For Windows users: Open Powershell as an adminstrator. In the PowerShell window, run the following commands to navigate to the Telegraf folder and move the `telegraf.exe` and `telegraf.conf` files:
