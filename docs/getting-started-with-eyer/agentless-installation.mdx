...
Telegraf is an open-source agent that collects metrics from your applications. To run your Telegraf as a service, download the Eyer `telegraf.conf` file from [this link](https://drive.google.com/uc?export=download&id=1HR1ODNDn57f1cXzsolWZ6p3HBndJJCwl).

Next, navigate to your InfluxDB Telegraf folder, typically located at `C:\Program Files\InfluxData\telegraf\telegraf-1.30.1\telegraf`. Replace the existing `telegraf.conf` file with the downloaded Eyer `telegraf.conf` file.
